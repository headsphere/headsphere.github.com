...
```{r echo=FALSE}
u <- totalBusyTime/simTime       # utilization B/T
...
colnames(m) = c("utilization B/T", "mean queue length", "traffic intensity", "mean residence time", "estimated queue length ")
kable(m)
...
library(knitr)
m <- as.matrix(t(c(u, N, x, r, q)))
colnames(m) = c("utilization B/T", "mean queue length", "mean throughput C/T", "mean residence time", "estimated queue length ")
kable(m)
